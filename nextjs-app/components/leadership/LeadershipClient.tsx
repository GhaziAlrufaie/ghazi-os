    ...
    <div className="app" style={{ direction: 'rtl' }}>
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-head">
          <div className="logo">
            ...
            <div className="logo-text">
              <div className="logo-name">Ghazi OS</div>
              <div className="logo-sub">Command Center</div>
            </div>
          </div>
          {/* Quick stats pill */}
          <div className="side-pill">
            <span className="pill-label">⚡ اليوم</span>
            <span className="pill-value">
              {activeTasks.filter(t => t.status === 'done').length} منجز
            </span>
          </div>
        </div>
        ...
        <nav className="nav">
          ...
          <Link href="/leadership" className="nav-item active">
            <span className="nav-icon">🏗️</span>
            <span className="nav-label">المركز القيادي</span>
          </Link>
          <Link href="/brands" className="nav-item">
            <span className="nav-icon">✨</span>
            <span className="nav-label">البراندات</span>
            <span className="nav-badge">{brands.length}</span>
          </Link>
          <Link href="/tasks" className="nav-item">
            <span className="nav-icon">📋</span>
            <span className="nav-label">المهام</span>
            {activeTasks.length > 0 && (
              <span className="nav-badge alert">{activeTasks.length}</span>
            )}
          </Link>
          <Link href="/projects" className="nav-item">
            <span className="nav-icon">📁</span>
            <span className="nav-label">المشاريع</span>
            <span className="nav-badge">{projects.length}</span>
          </Link>
          <Link href="/decisions" className="nav-item">
            <span className="nav-icon">⚖️</span>
            <span className="nav-label">القرارات</span>
            {decisions.length > 0 && (
              <span className="nav-badge alert">{decisions.length}</span>
            )}
          </Link>
          <Link href="/sales" className="nav-item">
            <span className="nav-icon">📈</span>
            <span className="nav-label">المبيعات</span>
          </Link>

          <div className="nav-group">الخاص</div>
          <Link href="/personal" className="nav-item">
            <span className="nav-icon">👤</span>
            <span className="nav-label">الشخصي</span>
          </Link>
          <Link href="/worlds" className="nav-item">
            <span className="nav-icon">🌍</span>
            <span className="nav-label">عوالمي</span>
          </Link>
          <Link href="/finance" className="nav-item">
            <span className="nav-icon">💰</span>
            <span className="nav-label">الماليات</span>
          </Link>
        </nav>
        ...
        <div className="side-foot">
          ...
          <div className="user-info">
            <div className="user-name">غازي</div>
            <div className="user-status">نشط الآن</div>
          </div>
        </div>
      </aside>
